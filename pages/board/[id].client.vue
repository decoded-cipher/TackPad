<template>
  ...
  >
    ...
    <!-- Fixed Controls -->
    <div
      class="fixed bottom-4 left-1/2 transform -translate-x-1/2 flex items-center bg-white rounded-xl shadow-lg px-2 sm:px-4 py-1 sm:py-2 gap-2 sm:gap-6"
    >
      <button
...
<style>
::view-transition-old(selected-widget),
::view-transition-new(selected-widget) {
  animation-duration: 0.5s;
}

::view-transition-old(selected-widget) {
  animation: 0.4s ease-out forwards delete-animation;
}


@keyframes delete-animation {
  from { transform: scale(1) translateY(0); opacity: 1;}
  to { transform: scale(0) translateY(250px); opacity: 0.2;}
}
